"""
Meta-learning implementation for SAC algorithm.

This module provides MAML and FOMAML implementations specifically designed
for SAC (Soft Actor-Critic) algorithm.
"""
...
from .sac_learning import SAC, DiscreteActor, DiscreteCritic

logger = logging.getLogger(__name__)


class MetaSAC(MetaLearningWrapper):
    """
    Meta-learning wrapper for SAC algorithm.
    
    This class applies MAML or FOMAML to SAC, enabling the algorithm
    ...
    def __init__(
        ...
        Args:
            ...
            auto_entropy: Whether to automatically tune entropy
        ...
        if meta_method.lower() == "maml":
            self.meta_learner = MAMLSAC(
                base_sac, meta_lr, inner_lr, num_inner_steps, device
            )
        elif meta_method.lower() == "fomaml":
            self.meta_learner = FOMAMLSAC(
                base_sac, meta_lr, inner_lr, num_inner_steps, device
            )
        else:
    ...
    def train_meta_on_tasks(self, tasks: List[Dict[str, Any]], num_meta_epochs: int = 100) -> Dict[str, List[float]]:
        ...
        """
        logger.info(f"Starting {self.meta_method.upper()} meta-learning training for SAC")
...
class MAMLSAC(MAML):
    ...
    def _inner_loop_step(self, task_data: Dict[str, Any], params: List[torch.Tensor]) -> torch.Tensor:
        ...
        # Collect data and compute SAC loss
        loss = self._compute_sac_loss(env)
    ...
    def _compute_sac_loss(self, env: gym.Env) -> torch.Tensor:
        """
        Compute SAC loss from environment data.
        ...
        obs_batch = torch.FloatTensor([self.base_algorithm._flatten_observation(self.base_algorithm.replay_buffer[i][0]) for i in indices]).to(self.device)
        act_batch = torch.LongTensor([self.base_algorithm.replay_buffer[i][1] for i in indices]).to(self.device)
        rew_batch = torch.FloatTensor([self.base_algorithm.replay_buffer[i][2] for i in indices]).to(self.device)
        next_obs_batch = torch.FloatTensor([self.base_algorithm._flatten_observation(self.base_algorithm.replay_buffer[i][3]) for i in indices]).to(self.device)
        done_batch = torch.FloatTensor([self.base_algorithm.replay_buffer[i][4] for i in indices]).to(self.device)
        
        # Update critics
        with torch.no_grad():
            next_actions, next_log_probs, _ = self.base_algorithm.actor.sample(next_obs_batch)
            target_q1 = self.base_algorithm.critic1_target(next_obs_batch, next_actions)
            target_q2 = self.base_algorithm.critic2_target(next_obs_batch, next_actions)
            target_q = torch.min(target_q1, target_q2)
            target_q = rew_batch + (1 - done_batch) * self.base_algorithm.gamma * (target_q - self.base_algorithm.alpha * next_log_probs)
        ...
        # Update actor
        actions, log_probs, _ = self.base_algorithm.actor.sample(obs_batch)
        q1 = self.base_algorithm.critic1(obs_batch, actions)
        q2 = self.base_algorithm.critic2(obs_batch, actions)
        q = torch.min(q1, q2)
        actor_loss = (self.base_algorithm.alpha * log_probs - q).mean()
    ...
    def _compute_loss_for_task(self, task_data: Dict[str, Any]) -> torch.Tensor:
        ...
        """
        env = task_data['env']
        return self._compute_sac_loss(env)


class FOMAMLSAC(FOMAML):
    """
    FOMAML implementation specifically for SAC.
    
    This class inherits from FOMAML and implements SAC-specific
